document.addEventListener('DOMContentLoaded', () => {
    ...
    function deleteNote(id) {
        ...
        noteDatabase = noteDatabase.filter(element => element != noteDatabase[indexDatabase]);
    ...
    function renderNote(noteDatabase) {
        ...
        noteDatabase.forEach((noteObject) => {
            const note = createNote(noteObject.id, noteObject.bgColor, noteObject.title, noteObject.content, noteObject.pinned);
    ...
    function selectColor(bgColor) {
        color.forEach(col => {
            const getClassColorElement = document.querySelector(`.${col.classList[1]}`);
    ...
    editableTextArea[0].addEventListener('input', () => {
        if (editableTextArea[0].innerText.trim() === '' && editableTextArea[1].innerText.trim() === '') {
            saveDialogBtn.disabled = true;
        } else {
            saveDialogBtn.disabled = false;
        }
    ...
    color.forEach(element => {
        element.addEventListener('click', () => {
            ...
            const getClassColorElement = document.querySelector(`.${element.classList[1]}`);
            const getBgColor = getComputedStyle(getClassColorElement).backgroundColor;
    ...
    searchBtn.addEventListener('click', () => {
        ...
        noteDatabase.forEach(note => {
            if (note.title.toLowerCase().includes(inputText) || note.content.toLowerCase().includes(inputText)) {
                dataSet.add(note);
            }
        });
        renderNote(dataSet)
